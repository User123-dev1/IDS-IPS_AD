...
class SecurePacketCapture:
    ...
    # Security limits
    MAX_PACKETS = 100000  # Maximum packets to capture
    MAX_PACKET_SIZE = 65535  # Maximum packet size to process
    MAX_QUEUE_SIZE = 10000  # Maximum packet queue size
    MAX_PAYLOAD_PREVIEW = 100  # Maximum bytes to preview
    RATE_LIMIT = 10000  # Max packets per second

    # Industrial protocol ports (read-only, trusted data)
    INDUSTRIAL_PORTS = {
        502: 'Modbus/TCP',
        20000: 'DNP3',
        2404: 'IEC-104',
    ...
    def start_capture(self,
        ...
        Args:
            ...
            filter_expr: BPF filter expression for capture
            callback: Function to call for each packet
            timeout: Maximum capture time in seconds

        Security Notes:
            - Runs in separate thread for isolation
            - Validates all packet data before processing
        ...
        # Validate filter expression (prevent injection)
    ...
    def _capture_worker(self, interface, filter_expr, callback, timeout):
        """Worker thread for packet capture with error isolation"""
        try:
            # Build safe filter for industrial protocols
            safe_filter = self._build_industrial_filter(filter_expr)

            start_time = time.time()

            def packet_handler(packet):
                """Safe packet handler with validation"""
                # Check timeout
    ...
    def _safe_parse_packet(self, packet) -> Optional[PacketInfo]:
        """
        Safely parse packet with validation and bounds checking

        Security measures:
        - Validates packet structure
        - Checks size limits
        - Sanitizes all string data
        - No execution of packet data
        ...
        try:
            ...
            # Get safe payload preview (sanitized)
    ...
    def _get_safe_payload(self, packet) -> str:
        ...
        Security: Only returns printable ASCII, limited length
        """
        try:
            if packet.haslayer(Raw):
                ...
                # Only keep printable ASCII (prevent injection)
                safe_payload = ''.join(
    ...
    def _validate_bpf_filter(self, filter_expr: str) -> bool:
        ...
        Security: Prevent injection attacks via malicious filters
        """
        if not filter_expr:
            return True
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'protocols': dict(self.statistics),
            'queue_size': self.packet_queue.qsize()
        }

    def get_next_packet(self, timeout: float = 1.0) -> Optional[PacketInfo]:
